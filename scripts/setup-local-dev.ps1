...
#
# IMPORTANT: No hardcoded values - all sensitive values should be provided via parameters or Azure CLI
...
if (-not (Test-Path "api/local.settings.json")) {
    ...
    if ($account) {
        ...
        # Require Cosmos account name for automated retrieval
        if (-not $CosmosAccountName) {
            Write-Host "No Cosmos account name provided. Use -CosmosAccountName parameter for automated setup." -ForegroundColor Yellow
        } else {
            try {
                ...
                if ($StorageAccountName) {
                    $storageConnectionString = az storage account show-connection-string --name $StorageAccountName --resource-group $ResourceGroupName --query "connectionString" -o tsv 2>$null
                }
    ...
    if (-not $cosmosKey) {
        ...
        Write-Host "  2. Find your Cosmos DB account" -ForegroundColor Gray
        ...
        Write-Host "  1. Find your storage account in Azure Portal" -ForegroundColor Gray
...
if (-not (Test-Path ".env.local")) {
    ...
    if (-not $cosmosEndpoint) {
        $cosmosEndpoint = Read-Host "Enter Cosmos DB Endpoint for frontend"
    }
    if (-not $AzureClientId) {
        Write-Host ""
        Write-Host "Azure AD Client ID is required for authentication." -ForegroundColor Yellow
        Write-Host "  1. Go to Azure Portal -> Azure Active Directory -> App registrations" -ForegroundColor Gray
        Write-Host "  2. Find or create your app registration" -ForegroundColor Gray
        Write-Host "  3. Copy the Application (client) ID" -ForegroundColor Gray
        Write-Host ""
        $AzureClientId = Read-Host "Enter Azure AD Client ID"
    }
