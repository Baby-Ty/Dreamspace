...
if (!process.env.COSMOS_ENDPOINT || !process.env.COSMOS_KEY) {
  ...
  if (fs.existsSync(localSettingsPath)) {
    try {
      ...
      // Silently fail if can't parse local.settings.json
    }
  }
      ...
      envContent.split('\n').forEach(line => {
        ...
        if (trimmed && !trimmed.startsWith('#')) {
          ...
          if (key && valueParts.length > 0) {
            ...
            // Map VITE_ prefixed vars to non-prefixed for Node.js scripts
            if (key.startsWith('VITE_COSMOS_')) {
              const nodeKey = key.replace('VITE_', '');
              if (!process.env[nodeKey]) {
                process.env[nodeKey] = value;
              }
            } else if (key.startsWith('COSMOS_')) {
              if (!process.env[key]) {
...
// Check for environment variables (same as Azure Functions use)
const endpoint = process.env.COSMOS_ENDPOINT;
const key = process.env.COSMOS_KEY;

if (!endpoint || !key) {
  console.error('❌ Error: COSMOS_ENDPOINT and COSMOS_KEY environment variables are required');
  console.error('\nThe script tried to load from:');
  console.error('  1. api/local.settings.json (Azure Functions style)');
  console.error('  2. .env file (if exists)');
  console.error('  3. Environment variables');
  console.error('\nFound:');
  console.error(`  COSMOS_ENDPOINT: ${endpoint ? '✅ Set' : '❌ Not set'}`);
  console.error(`  COSMOS_KEY: ${key ? '✅ Set' : '❌ Not set'}`);
  console.error('\nSet them in api/local.settings.json (recommended):');
  console.error('  {');
  console.error('    "Values": {');
  console.error('      "COSMOS_ENDPOINT": "https://your-account.documents.azure.com:443/",');
  console.error('      "COSMOS_KEY": "your-primary-key"');
  console.error('    }');
  console.error('  }');
  console.error('\nOr set in PowerShell:');
  console.error('  $env:COSMOS_ENDPOINT="https://your-account.documents.azure.com:443/"');
  console.error('  $env:COSMOS_KEY="your-primary-key"');
  process.exit(1);
}
  ...
  for (let i = 0; i < Math.min(count, selectedTitles.length); i++) {
    ...
    if (Math.random() > 0.3) {
      ...
      goals.push({
        ...
        completedAt: weeksRemaining === 0 ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString() : null,
        createdAt: createdAt
      });
    }
    ...
    dreams.push({
      ...
      history: [],
      completed: false,
      isPublic: true, // Make dreams public so goals are visible to team
      image: '', // Can be populated with Unsplash URLs if needed
      createdAt: createdAt,
  ...
  try {
    ...
    for (const user of mockUsers) {
      ...
      try {
        ...
        const dreamsDoc = {
          id: user.userId,
          userId: user.userId,
          dreams: dreams, // Use 'dreams' array (not 'dreamBook') to match UI structure
    ...
    for (const user of mockUsers) {
      ...
      try {
        ...
        // Support both 'dreams' (new) and 'dreamBook' (legacy) for compatibility
        const userDreams = dreamsDoc?.dreams || dreamsDoc?.dreamBook || [];
